...
#         feat_normal_bottom, scores, scores_nor_bottom, scores_nor_abn_bag, _,LAT_loss,triplet_loss = model(input, text,True,percent)  # b*32 x 2048
#
#         # 动态损失加权
#         a_mask = (sources == 0)
#         b_mask = (sources == 1)
...
def train(nloader, aloader, model, args, optimizer, viz, device, percent, logger=None, step=None):
    with torch.set_grad_enabled(True):
        ...
        # 获取数据并包含来源标记
        ninput, ntext, nlabel, sources_n = next(nloader)
        ainput, atext, alabel, sources_a = next(aloader)

        # 合并正常和异常数据
        input = torch.cat((ninput, ainput), 0).to(device)  # [64, 10, 32, 2048]
        text = torch.cat((ntext, atext), 0).to(device)  # [64, 32, 768]
        labels = torch.cat((nlabel, alabel), 0).to(device)  # [64]
        ...
        # 前向传播
        score_abnormal, score_normal, feat_select_abn, feat_select_normal, feat_abn_bottom, \
            feat_normal_bottom, scores, scores_nor_bottom, scores_nor_abn_bag, _, LAT_loss, triplet_loss = model(
            mixed_input, mixed_text, True, percent)

        # --- 关键修改：聚合片段级分数为样本级分数 ---
        batch_size_total = mixed_input.size(0)  # 64
        num_snippets = scores.size(1)  # 32
        scores = scores.view(batch_size_total, num_snippets)  # [64, 32]
        scores = torch.mean(scores, dim=1)  # [64] 对每个视频的32个片段取平均

        # 计算分类损失（样本级）
        loss_criterion = torch.nn.BCEWithLogitsLoss()
        loss_cls = loss_criterion(scores, mixed_labels)  # 输入和标签形状均为 [64]

        # 其他损失项（保持原有逻辑）
        abn_scores = scores[batch_size:]  # 仅用异常分支的分数计算稀疏和平滑损失
        loss_sparse = sparsity(abn_scores, batch_size, 8e-3)
        loss_smooth = smooth(abn_scores, 8e-4)

        # 组合总损失
        total_loss = loss_cls + loss_smooth + loss_sparse + args.beta * LAT_loss + triplet_loss
        ...
        if logger:
            logger.log(f"Epoch {step}  loss: {total_loss.item():.4f}  smooth loss: {loss_smooth.item():.4f}  sparsity loss: {loss_sparse.item():.4f}")
