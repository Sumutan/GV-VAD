...
#LAT
def calculate_abn(abn_bank,inputs,percent):
    inputs = inputs.unsqueeze(2).repeat(1,1,26,1)
...
def calculate_triplet(normal_feature,normal_score,abnormal_feature,abnormal_score):
    _,anc_index = torch.topk(normal_score, 30, dim=0, largest=False)
    _, pos_index = torch.topk(normal_score, 30, dim=0, largest=False)
    #_,pos_index = torch.topk(abnormal_score, 30, dim=0, largest=False)
    _,neg_index = torch.topk(abnormal_score, 30, dim=0, largest=True)
...
class Model(nn.Module):
    ...
    def forward(self, inputs, text,percent, is_training=False):
        #percent is a dynamic threshold to select abnormal features,accroding to iteration
        if is_training:
            abn_index = calculate_abn(self.abn_bank, text[self.batch_size:,4],percent)
        ...
        if is_training:
        # #triplet loss
            #anc_feature,pos_feature,neg_feature = calculate_triplet(normal_features,normal_scores,abnormal_features,abnormal_scores)
            #triplet_loss = self.triplet(anc_feature,pos_feature,neg_feature)
        ...
        # therefore, we use score_abnormal and score_normal during training and scores during inference
            return score_abnormal, score_normal, feat_select_abn, feat_select_normal, feat_select_abn, feat_select_abn, scores, feat_select_abn, feat_select_abn, feat_magnitudes, abn_smooth_loss
        else:
