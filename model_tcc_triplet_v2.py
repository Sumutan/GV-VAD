...
def calculate_abn(abn_bank,inputs,capacity,step):
    ...
    similarity_all,index = torch.max(similarity_all,dim=-1)[0],torch.max(similarity_all,dim=-1)[1]
    if step < 8:
        global prompt_count
        index = index.view(-1)
        prompt_count[-1] += len(index)
        for abnormal in index:
            prompt_count[abnormal] +=1
    if step == 8:
        prompt_count = [x/prompt_count[-1] for x in prompt_count]
        print(prompt_count)
        exit(0)
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        if args.dataset == 'ucfcrime' or args.dataset == 'violence':
            self.abn_bank = torch.tensor(np.load('save/Crime/UCF_abnormal_text_15.npy')).to('cuda')
    ...
    def forward(self, inputs, text,step, is_training=False):
        #percent is a dynamic threshold to select abnormal features,accroding to iteration
        if is_training:
            abn_index,n_index = calculate_abn(self.abn_bank, text[self.batch_size:,4],self.abn_bank.shape[0],step)
        else:
            abn_index,n_index = calculate_abn(self.abn_bank, text[:, 4], self.abn_bank.shape[0],step)
        ...
        # therefore, we use score_abnormal and score_normal during training and scores during inference
            ...
            return inputs.squeeze(0),features, score_abnormal, score_normal, feat_select_abn, feat_select_normal, feat_select_abn, feat_select_abn, scores,scores_10crop, feat_select_abn, feat_select_abn, feat_magnitudes
