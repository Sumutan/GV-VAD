...
def calculate_abn(abn_bank,inputs,capacity,videoname):
    ...
    if videoname:
        similarity = similarity_all.squeeze(0).repeat(16,1).cpu().numpy()
        data = pd.DataFrame(similarity)
        writer = pd.ExcelWriter("excel/{}.xlsx".format(videoname))
    ...
    def __init__(self, args):
        ...
        if self.feature_group == 'both':
            if args.fusion == 'concat':
                ...
                self.abn_bank = torch.tensor(np.load('save/Crime/UCF_abnormal_text.npy')).to('cuda')
