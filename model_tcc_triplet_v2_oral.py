...
#LAT 先取最大相似异常后划定阈值，一个实例最大相似度匹配上库里才是异常
def calculate_abn(abn_bank,inputs,capacity,videoname):
    inputs = inputs.unsqueeze(2).repeat(1,1,capacity,1)
    abn_bank = abn_bank.unsqueeze(0)
    similarity_all = F.cosine_similarity(inputs,abn_bank,dim=-1)
    similarity_all = torch.max(similarity_all,dim=-1)[0]
    std,mean = torch.std_mean(similarity_all)
    threshold = mean+(1)*std
    #threshold = 0.5
    c = torch.where(similarity_all>threshold,torch.ones_like(similarity_all),torch.zeros_like(similarity_all))
    #c = torch.sum(c,dim=-1)
    c[ c > 0 ] = 1
    return c.unsqueeze(-1),(1-c).unsqueeze(-1)

#先算所有相似度划定阈值后算总和，可能有一个实例匹配上多个异常
# def calculate_abn(abn_bank,inputs,capacity):
#     inputs = inputs.unsqueeze(2).repeat(1,1,capacity,1)
#     abn_bank = abn_bank.unsqueeze(0)
#     similarity_all = F.cosine_similarity(inputs,abn_bank,dim=-1)
#     std,mean = torch.std_mean(similarity_all)
...
def calculate_triplet(normal_feature,normal_score,abnormal_feature,abnormal_score,capacity,abn_index,n_index):
    A, B, C, D = abnormal_feature.size()
    normal_feature = normal_feature.permute(1,2,0,3).reshape(B*C,-1)
    abnormal_feature = abnormal_feature.permute(1,2,0,3).reshape(B*C,-1)
    _,anc_index = torch.topk(normal_score.view(-1,1), capacity, dim=0,largest=False)
    _, pos_index = torch.topk(abnormal_score.view(-1, 1), capacity, dim=0, largest=False)
    _,neg_index = torch.topk((abnormal_score*abn_index).view(-1,1), capacity, dim=0, largest=True)
    #_, neg_index = torch.topk(abnormal_score.view(-1, 1), capacity, dim=0, largest=True)
    anc_feature = normal_feature[anc_index]
    #pos_feature = normal_feature[pos_index]
    pos_feature = abnormal_feature[pos_index]
    neg_feature = abnormal_feature[neg_index]
    return anc_feature,pos_feature,neg_feature
...
class Aggregate_text(nn.Module):  # MTN
    def __init__(self, len_feature):
        super(Aggregate_text, self).__init__()
        bn = nn.BatchNorm1d
        self.len_feature = len_feature
        self.conv_1 = nn.Sequential(
            nn.Conv1d(in_channels=len_feature, out_channels=int(len_feature/4), kernel_size=3,
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        if 'ucf' in args.dataset or args.dataset == 'violence':
            #self.abn_bank = torch.tensor(np.load('save/Crime/UCF_abnormal_text_29.npy')).to('cuda')
            self.abn_bank = torch.tensor(np.load('save/Crime/UCF_abnormal_text_word.npy')).to('cuda')
            self.num_segments = 32
        elif args.dataset == 'shanghai':
            self.abn_bank = torch.tensor(np.load('save/Shanghai/SHT_abnormal_text.npy')).to('cuda')
            self.num_segments = 32
        elif args.dataset == 'tad':
            self.abn_bank = torch.tensor(np.load('save/TAD/TAD_abnormal_text_30.npy')).to('cuda')
        ...
        if self.feature_group == 'both':
            ...
            self.fc0 = nn.Linear(args.feature_size, args.feature_size)
    ...
    def forward(self, inputs, text,videoname, is_training=False):
        ...
        if is_training:
            abn_index,n_index = calculate_abn(self.abn_bank, text[self.batch_size:,4],self.abn_bank.shape[0],None)
        else:
            abn_index,n_index = calculate_abn(self.abn_bank, text[:, 4], self.abn_bank.shape[0],videoname)
        ...
        if is_training:
            abn_smooth_loss = binary_CE_loss(1-abnormal_scores,abn_index)
        ...
        if is_training:
            ...
            feature_anchor, feature_positive, feature_negative = calculate_triplet(normal_features,normal_scores,abnormal_features,abnormal_scores,self.abn_bank.shape[0],abn_index,n_index)
            triplet_loss = self.gama * self.triplet(feature_anchor, feature_positive, feature_negative)
        ...
        # therefore, we use score_abnormal and score_normal during training and scores during inference
            return score_abnormal, score_normal, feat_select_abn, feat_select_normal, feat_select_abn, feat_select_abn, scores, feat_select_abn, feat_select_abn, feat_magnitudes, abn_smooth_loss, triplet_loss
        else:
            return inputs.squeeze(0),features, score_abnormal, score_normal, feat_select_abn, feat_select_normal, feat_select_abn, feat_select_abn, scores, feat_select_abn, feat_select_abn, feat_magnitudes
